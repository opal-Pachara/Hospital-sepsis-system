    ...
    <>
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between flex-shrink-0 relative overflow-hidden z-20" style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Admin & Testing Suite Shortcut Button */}
          {/* {onNavigateAdmin && (
            <button
              type="button"
              id="btn-header-admin-testing"
              onClick={onNavigateAdmin}
              title="ระบบผู้ดูแลและเครื่องมือทดสอบ (Admin & Testing Suite)"
              style={{
                padding: '5px 12px',
                borderRadius: '8px',
                fontSize: '11px',
          ...
          {/* System Logs Viewer Button */}
          <button
            type="button"
            id="btn-header-system-logs"
            onClick={() => setShowSystemLogs(true)}
            title="เปิดดู Timestamp Logs และสถานะฐานข้อมูล"
            style={{
              padding: '5px 12px',
              borderRadius: '8px',
              fontSize: '11px',
      ...
      {/* System Logs Modal (Timestamp Logs & DB Diagnostics) */}
      <SystemLogsModal
